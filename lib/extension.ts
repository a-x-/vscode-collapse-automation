...
export function activate(context: vscode.ExtensionContext): void {
		...
		for (const call of result.multiLine) {
			if (call.endLine > call.startLine) {
				...
				if (isNowVisible && !manuallyUnfoldedLines.has(call.startLine)) {
					...
					outputChannel.appendLine(`- User manually unfolded ${call.pattern} at line ${call.startLine + 1}: ${lineText.substring(0, 50)}...`);
					...
					const lineText = document.lineAt(call.startLine).text.trim();
					outputChannel.appendLine(`- User manually folded back ${call.pattern} at line ${call.startLine + 1}: ${lineText.substring(0, 50)}...`);
				}
			}
		}
	...
	const commandDisposable = vscode.commands.registerCommand('collapse-automation.activate', () => {
		if (vscode.window.activeTextEditor) {
			...
			analyzeAndFold(vscode.window.activeTextEditor.document, true); // true = manual command
		} else {
			vscode.window.showInformationMessage('No active editor found');
		}
	});

	context.subscriptions.push(disposable, openDisposable, activeEditorDisposable, visibleRangeDisposable, commandDisposable);
...
async function analyzeAndFold(document: vscode.TextDocument, isManualCommand: boolean = false): Promise<void> {
	...
	const savedSelections = editor.selections.map(sel => 
			...
			for (const call of functionCalls) {
				try {
					...
					if (!isManualCommand && manuallyUnfoldedLines.has(call.startLine)) {
						outputChannel.appendLine(`  - Skipped ${call.pattern} at line ${call.startLine + 1}: manually unfolded by user`);
						skippedManual++;
						continue;
					}
